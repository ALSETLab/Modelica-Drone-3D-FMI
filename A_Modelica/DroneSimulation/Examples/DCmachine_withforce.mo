...
equation
  ...
  connect(signalVoltage.p,dcpm. pin_ap)
    annotation (Line(points={{-20,86},{-8,86},{-8,62}},    color={0,0,255}));
  connect(ground.p,dcpm. pin_an)
    annotation (Line(points={{-29,62},{-20,62}},   color={0,0,255}));
  ...
  connect(revolute2.frame_a, frame_a1) annotation (Line(
      ...
      color={95,95,95},
      thickness=0.5));
  connect(limiter.u, u1) annotation (Line(points={{-46.8,76},{-80,76},{-80,78},
          {-120,78}}, color={0,0,127}));
  connect(dcpm.flange, revolute2.axis)
    annotation (Line(points={{-4,52},{42,52},{42,22}}, color={0,0,0}));
  annotation (Icon(coordinateSystem(preserveAspectRatio=false)), Diagram(
        coordinateSystem(preserveAspectRatio=false)));
end DCmachine_withforce;
