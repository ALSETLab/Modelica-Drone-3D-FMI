...
model DCMotor_with_Power "DC motor using DC machine from MSL"
  ...
  Modelica.Blocks.Nonlinear.Limiter limiter(uMax=1e8, uMin=0)
    annotation (Placement(transformation(extent={{-88,28},{-80,36}})));
  ...
  parameter Real V "Constant output value";
equation
  ...
  connect(position, limiter.u)
    annotation (Line(points={{-120,-60},{-96,-60},{-96,32},{-88.8,32}},
                                                  color={0,0,127}));
  ...
  connect(signalVoltage.n, dcpm.pin_an)
    annotation (Line(points={{-68,-4},{-68,-14}},  color={0,0,255}));
  connect(signalVoltage.p, dcpm.pin_ap)
    annotation (Line(points={{-56,-4},{-56,-14}},          color={0,0,255}));
  connect(ground.p, dcpm.pin_an)
    annotation (Line(points={{-77,-14},{-68,-14}}, color={0,0,255}));
  connect(potentialSensor.p, p1)
    annotation (Line(points={{-84,68},{-98,68},{-98,60},{-110,60}},
                                                  color={0,0,255}));
  connect(potentialSensor.phi, product.u1)
    annotation (Line(points={{-63,68},{-48,68}}, color={0,0,127}));
  connect(limiter.y, product.u2) annotation (Line(points={{-79.6,32},{-66,32},{
          -66,56},{-48,56}}, color={0,0,127}));
  connect(product.y, division.u1) annotation (Line(points={{-25,62},{12,62},{12,
          48},{6,48}}, color={0,0,127}));
  connect(division.u2, const.y)
    annotation (Line(points={{6,36},{15,36}}, color={0,0,127}));
  connect(division.y, signalVoltage.v)
    annotation (Line(points={{-17,42},{-62,42},{-62,3.2}}, color={0,0,127}));
