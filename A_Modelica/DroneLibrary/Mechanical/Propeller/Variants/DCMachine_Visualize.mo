...
model DCMachine_Visualize
  Motor.Variants.DCMotor_DCMachine dCMotor2_1(
    k=PropellerGain,
    VaNominal=VaNominal,
    IaNominal=IaNominal) annotation (Placement(transformation(extent={{-80,-20},{-40,20}})));
  ...
  Blades.Variants.Blades_Visualize blades_Visualization annotation (Placement(transformation(extent={{50,6},{92,46}})));
...
equation
  connect(rotor1.Airframe, Airframe) annotation (Line(
      ...
      color={95,95,95},
      thickness=0.5));
  connect(rotor1.Blade, blades_Visualization.Input) annotation (Line(
