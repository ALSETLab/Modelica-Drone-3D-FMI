...
model controlModule_Power
  ...
  DroneLibrary.Blocks.Control.discretePID discretePID(
    ...
    kd=z_kd,
    kp=z_kp,
    samplePeriod=samplePeriod) annotation (Placement(transformation(extent={{-38,-40},{-18,-20}})));
  DroneLibrary.Blocks.Routing.RealExtract realExtract(index=1) annotation (Placement(transformation(extent={{-80,20},{-60,40}})));
  Modelica.Blocks.Interfaces.RealInput position[3]
    annotation (Placement(transformation(extent={{-140,-20},{-100,20}}),
        iconTransformation(extent={{-140,-20},{-100,20}})));
  DroneLibrary.Blocks.Routing.RealExtract realExtract1(index=2) annotation (Placement(transformation(extent={{-80,-10},{-60,10}})));
  DroneLibrary.Blocks.Routing.RealExtract realExtract2(index=3) annotation (Placement(transformation(extent={{-80,-40},{-60,-20}})));
  DroneLibrary.Blocks.Control.discretePID discretePID1(
    kp=y_kp,
    kd=y_kd,
    samplePeriod=samplePeriod,
    ki=y_ki) annotation (Placement(transformation(extent={{-40,-10},{-20,10}})));
  DroneLibrary.Blocks.Control.discretePID discretePID2(
    kd=gyro_x_kd,
    kp=gyro_x_kp,
    samplePeriod=samplePeriod,
    ki=gyro_x_ki) annotation (Placement(transformation(extent={{46,20},{66,40}})));
  DroneLibrary.Blocks.Routing.RealExtract realExtract3(index=2) annotation (Placement(transformation(extent={{-4,-4},{4,4}}, origin={14,-72})));
  DroneLibrary.Blocks.Routing.RealExtract realExtract4(index=2) annotation (Placement(transformation(extent={{-52,-68},{-44,-60}})));
  parameter Real maxTilt=2 "Upper limits of input signals";
  DroneLibrary.Blocks.Routing.RealExtract realExtract5 annotation (Placement(transformation(extent={{-44,-76},{-36,-68}})));
  DroneLibrary.Blocks.Control.discretePID discretePID3(
    kd=x_kd,
    kp=x_kp,
    samplePeriod=samplePeriod,
    ki=x_ki) annotation (Placement(transformation(extent={{-40,20},{-20,40}})));
  Modelica.Blocks.Nonlinear.Limiter limiter(uMax=0.523)
    annotation (Placement(transformation(extent={{0,20},{20,40}})));
  DroneLibrary.Blocks.Routing.RealExtract realExtract6(index=1) annotation (Placement(transformation(extent={{-60,-60},{-52,-52}})));
  Modelica.Blocks.Nonlinear.Limiter limiter1(uMax=0.523)
    annotation (Placement(transformation(extent={{0,-10},{20,10}})));
  DroneLibrary.Blocks.Control.discretePID discretePID4(
    kd=gyro_y_kd,
    kp=gyro_y_kp,
    samplePeriod=samplePeriod,
    ki=gyro_y_ki) annotation (Placement(transformation(extent={{32,-10},{52,10}})));
  DroneLibrary.Blocks.Routing.RealExtract realExtract7(index=1) annotation (Placement(transformation(extent={{-4,-4},{4,4}}, origin={14,-60})));
  DroneLibrary.Blocks.Routing.RealExtract realExtract8(index=3) annotation (Placement(transformation(
        extent={{-4,-4},{4,4}},
        rotation=90,
        origin={-28,58})));
  DroneLibrary.Blocks.Control.discretePID discretePID5(
    kd=Yaw_kd,
    kp=Yaw_kp,
    ki=Yaw_ki,
    samplePeriod=samplePeriod) annotation (Placement(transformation(extent={{-38,70},{-18,90}})));
...
<p>This is a discrete PID controller. It has inputs that account for the dictated position of the quadcopter and yaw. It compares the actual XYZ position of the quadcopter to the commanded XYZ position and controls the current applied to each of the motors as a result.</p>
<h4>This controller has an electrical input to estimate the power consumption.</h4>
</html>"));
